...
source("scripts/recent_esc_plots.R")

```

```{r set days, echo=FALSE}
tyee.day<-as.Date("2025-07-02")
tyee.date<-"July 2"
figures.x.date<-as.Date("2025-07-11")

# fence.day<-as.Date("2024-09-22")
# fence.date<-"September 22"

today<-as.Date("2025-07-02")
```
...
-   Pre-season forecasts
    ...
    -   Sockeye escapement estimates are tracking just below the median of returns from 1970-2024, with the TRTC model estimate increasing over the last few days, but still well below the pre-season forecast for early and average run-timing projections.
    -   Chinook are also being caught at Tyee, with the cumulative index increasing to greater than the 25th percentile to date. There is supporting information of low catch rates in FSC fisheries both in marine and in-river areas.
...
make.index.figures(daily,current,25,100,figures.x.date)
...
make.tyee.sockeye.escapement.plots(60000,100000,figures.x.date)

```

## Daily and cumulative sockeye escapement for recent (2021-2024) years vs. 2025

```{r, fig.height=7, fig.cap="Daily and cumulative sockeye escapement for 2021-2025"}
...
make.index.figures(daily,current,15,200,figures.x.date)
```
