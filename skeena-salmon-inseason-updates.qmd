...
tyee.day<-as.Date("2024-07-07")
tyee.date<-"July 7"
fence.day<-as.Date("2024-07-07")
fence.date<-"July 7"
```
...
-   To date 191,955 sockeye are estimated to have passed by Tyee, with about 9.9% of the run through to date on average (4.0% if 1-week late, 22% if one week early). Sockeye escapement has picked up over the last week and is now tracking well above the median daily run through (Figures 1-4).

-   We are getting closer to the mid-point of the run, but at \< 10% on average timing, it is still very early to make predictions on the final TRTC, however the excel model is increasing for all 3 run-timings, suggesting that the run may be coming in later than average. The current run size prediction is 1.9M on average run-timing (Figure 5).
