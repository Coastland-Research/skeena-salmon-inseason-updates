...
tyee.day<-as.Date("2024-07-18")
tyee.date<-"July 18"
fence.day<-as.Date("2024-07-18")
fence.date<-"July 18"
today<-as.Date("2024-07-18")
```
...
-   Tyee Test Fishery
    ...
    -   To date 582,080 sockeye are estimated to have passed by Tyee, with about 31.7% of the run through to date on average (16.1% if 1-week late, 52.2% if one week early). Sockeye escapement is not picking up for a large run and is continuing to be variable (Figures 1-5). Including commercial catch, the TRTC is around 590,000 (not including marine food fish and catch from the last few days).

    -   At \~30% on average timing, we are gaining a bit more confidence in predicting the final TRTC. The excel model was increasing for all 3 run-timings, however it stabilized briefly and now is decreasing significantly, especially in the late timing prediction. This may indicate a weaker mid-run and more average run-timing. We are also falling off the 2023 run now.

    -   The current run size prediction is 1.9M on average run-timing (3.3M if one week late and 1.2M if 1 week early) (Figure 6). The next week will be important as we move towards larger tides and towards the peak of the run, and will tell if we are tracking a 2022, 2023 or a something in between run. We should be seeing daily runs of between 30,000 and 60,000 to keep pace with the 2023 return.

-   Babine Sockeye

    -   The Babine River fence started operations on July 9th. To date the total run is 939 large sockeye, with 1 jack sockeye to date. More details will be added here shortly along with 2 forecast methods.

    -   The Babine comparison with Tyee will be updated as more sockeye arrive at the Babine fence.

-   Tyee Other Species

    -   The Chinook index remains very low and available evidence suggests a very low run.
...
-   Commercial Fisheries
    ...
    -   Commercial fishing has started, with an Area C gillnet opening on July 13 - catch numbers will be updated shortly but preliminary sockeye catch is 7,173 pieces.
...
b1<-ggplot(daily.babine%>%filter(period!="2024"),aes(x=Date,y=sockeye,color=period))+
  geom_line(linetype="longdash",alpha=.75)+
...
#| echo: false
#| warning: false
#| fig-cap: "Top panel: Total run of large sockeye (including harvest at the fence) to the Babine fence for 2024 compared to historical and recent averages." 
#| include: true
#| fig-height: 5
