...
tyee.day<-as.Date("2024-07-10")
tyee.date<-"July 10"
fence.day<-as.Date("2024-07-10")
fence.date<-"July 10"
```
...
-   To date 315,698 sockeye are estimated to have passed by Tyee, with about 14% of the run through to date on average (5.9% if 1-week late, 29.1% if one week early). Sockeye escapement has picked up over the last week and is now tracking well above the median daily run (Figures 1-5).

-   We are getting closer to the mid-point of the run, but at ~15% on average timing, it is still very early to make predictions on the final TRTC, however, the excel model is increasing for all 3 run-timings, suggesting that the run may be coming in later than average. The current run size prediction is 2.2M on average run-timing (4.7M if one week late and 1.2M if 1 week early) (Figure 6).
...
#| label: fig-daily-cumulative-boxplot
#| echo: false
#| message: false
#| warning: false
