---
title: "Skeena Salmon In-Season Update for SFNTC"
...
tyee.day<-as.Date("2024-07-14")
tyee.date<-"July 14"
fence.day<-as.Date("2024-07-14")
fence.date<-"July 14"
```
...
-   To date 481,284 sockeye are estimated to have passed by Tyee, with about 22.0% of the run through to date on average (9.9% if 1-week late, 40.2% if one week early). Sockeye escapement has picked up over the last week and is continuing to be variable (Figures 1-5).

-   We are getting closer to the mid-point of the run, but at \~22% on average timing, it is still early to make predictions on the final TRTC. The excel model was increasing for all 3 run-timings, suggesting that the run may be coming in later than average, however it has since stabilized, for now. The current run size prediction is 2.2M on average run-timing (4.3M if one week late and 1.3M if 1 week early) (Figure 6). The next week will be important as we move towards larger tides and towards the peak of the run, and will tell if we are tracking a 2022, 2023 or someething in between run.

-   We are currently moving towards smaller tides which may influence the migration of salmon into the Skeena River [(Tides section)](#tide-information-at-port-edward)

-   The Babine fence started operations July 9, with 22 large sockeye through. The Babine section of the report will be udpated in the next few days, along with the Babine-Tyee comparison.

-   The Kitwanga fence started operations last week, with counts starting as of July 10.

-   Commercial fishing has started, with an Area C gillnet opening on July 13.

-   Appendix A will include information on the other species caught in the Area 4 commercial fishery and CPUEs and will be updated when information becomes available.
...
Currently, daily run estimates are tracking close to the last 2 years, and considerably higher than 2020 and 2021. See the next figure that shows the cumulative run through and the full season totals (unadjusted).
...
Currently, the 2024 run is tracking close to the last 2 years, and considerably higher than 2020 and 2021.
...
The Babine fence started operations on July 9, with no salmon passing through to date. More data will be added to this section shortly.
...
fence<-read_excel("data/tyee and catch and fence in-season updates 2024.xlsx",sheet="Babine")
...
#| message: false
#| warning: false
#| fig-cap: "Daily run for Chinook salmon through Tyee (uncorrected) from 1956-2023. 2023 run is shown in purple."
#| fig-height: 5
library(viridis)
...
gg.daily.quants<-gg.daily %>%
  group_by(Date) %>%
  summarise(quantile10 = scales::percent(.1), 
            quantile25 = scales::percent(.25), 
            quantile50 = scales::percent(.50),
            quantile75 = scales::percent(.75),
            quantile90 = scales::percent(.9),
            per10 = quantile(Chinook,.1, na.rm = TRUE), 
...
ggplot(gg.daily,aes(x=Date,y=Chinook,group=Date))+
  geom_boxplot(color="grey90")+
    geom_line(data=gg.daily.quants,aes(x=Date, y=per10_90, group=quantile10_90),color="#1F968BFF",linewidth = 0.4,linetype ="dashed") +
  geom_line(data=gg.daily.quants,aes(x=Date, y=per25, group=quantile25),color="#95D840FF", linewidth = .6, linetype = "dashed") +
  geom_line(data=gg.daily.quants,aes(x=Date, y=per50, group=quantile50),color="#39568CFF", linewidth = 1, linetype = "dashed") +
    geom_line(data = gg.daily %>% filter(Year == 2024), aes(x = Date, y = Chinook, group = 1),color = "purple", linewidth = 1)+
  theme_bw()
...
ggplot(gg.daily, aes(x = Date, y = Chinook, group = Date)) +
  geom_boxplot(color = "grey90") +
  geom_line(data = gg.daily.quants, aes(x = Date, y = per10, group = quantile10, color = "10th Percentile"), linewidth = 0.4, linetype = "dashed") +
  geom_line(data = gg.daily.quants, aes(x = Date, y = per25, group = quantile25, color = "25th Percentile"), linewidth = 0.6, linetype = "dashed") +
  geom_line(data = gg.daily.quants, aes(x = Date, y = per50, group = quantile50, color = "50th Percentile"), linewidth = 1, linetype = "dashed") +
  geom_line(data = gg.daily.quants, aes(x = Date, y = per75, group = quantile75, color = "75th Percentile"), linewidth = 0.6, linetype = "dashed") +
  geom_line(data = gg.daily.quants, aes(x = Date, y = per90, group = quantile90, color = "90th Percentile"), linewidth = 0.4, linetype = "dashed") +
  geom_line(data = gg.daily %>% filter(Year == 2024), aes(x = Date, y = Chinook, group = 1, color = "2024 Data"), linewidth = 1) +
  scale_color_manual(name = "", values = c("10th Percentile" = "#1F968BFF", "25th Percentile" = "#95D840FF", "50th Percentile" = "#39568CFF", "75th Percentile" = "#95D840FF", "90th Percentile" = "#1F968BFF", "2024 Data" = "purple"), breaks = c("10th Percentile", "25th Percentile", "50th Percentile", "75th Percentile", "90th Percentile", "2024 Data")) + 
  theme_bw()
...
#| message: false
#| warning: false
#| fig-cap: "Daily cumulative proportion of the Chinook salmon run through Tyee (uncorrected) from 1970-2023. The 2024 run estimates are shown by the coloured lines. Each line represents the proportion through based on a final season estimate shown in the legend. For example, the red line is based on a final TRTC of 1.5M, with the line showing the cumulative number of sockeye through on each date / 1.5M. Until we have the true final TRTC, we will not be able to calculate the true cumulative proportion. This is done to give an idea of run-timing and whether the pre-season forecast is likely to be close to the in-season estimate. The blue dashed line indicates the long-term average 50% through date."
#| fig-height: 5
####
# Read the CSV file
dailycm_chinook <- read_excel("data/tyee_indices_56_2023_allspecies.xlsx", sheet = "Large Chinook")

# Convert all year columns to numeric
year_cols <- names(dailycm_chinook)[4:ncol(dailycm_chinook)]
dailycm_chinook[year_cols] <- lapply(dailycm_chinook[year_cols], as.numeric)

# Reshape the dataframe to long format
gg_daily_cum <- dailycm_chinook %>%
  pivot_longer(cols = year_cols, names_to = "Year", values_to = "Chinook") %>%
mutate(Year=as.numeric(Year)) %>%
group_by(Year) %>%
mutate(chinook_cm = cumsum(Chinook), Date = as.Date((paste(MONTH,DAY,Year,sep= "-")), "%Y-%m-%d"))


ggplot(gg_daily_cum,aes(x=Date,y=Chinook,group=Date))+
  geom_point(color = "grey70") +
  #geom_boxplot(fill="white",color="grey70",alpha=.5)+
    geom_line(data=data2,aes(x=data2$date,y=p,color=estimate,group=estimate),linewidth=1)+
    geom_segment(aes(x=as.POSIXct("2024-06-01"),y=.5,xend=as.POSIXct("2024-07-24"),yend=.5),color="blue",linewidth=1,linetype="dashed")+
  geom_segment(aes(x=as.POSIXct("2024-07-24"),y=.5,xend=as.POSIXct("2024-07-24"),yend=0),color="blue",arrow = arrow(length=unit(.25, 'cm')),linewidth=1,linetype="dashed")+
    theme_bw()+
scale_x_date(date_breaks = "1 month",
             date_labels = "%B") +
  labs(color="Estimate",y="Chinook Cumulative Index")+
  theme(legend.position = "bottom")
...
#| message: false
#| warning: false
#| fig-cap: "Daily run for Chum salmon through Tyee (uncorrected) from 1956-2023. 2023 run is shown in purple."
#| fig-height: 5

daily<-read_excel("data/tyee_indices_56_2023_allspecies.xlsx", sheet = "Chum")

gg.daily<-daily%>%
  mutate_if(is.character, as.numeric) %>%
  pivot_longer("1956":"2023",names_to="Year",values_to="Chum") %>%
  mutate(Year=as.numeric(Year),
         Date = as.Date(paste("2023", MONTH, DAY, sep="-"), "%Y-%m-%d"))

ggplot(gg.daily,aes(x=Date,y=Chum,group=Date))+
  geom_boxplot(fill="white",alpha=.5)+
      geom_line(data=gg.daily%>%filter(Year==2023),aes(x=Date,y=Chum,group=1),
...
#| message: false
#| warning: false
#| fig-cap: "Daily cumulative proportion of the Chum salmon run through Tyee (uncorrected) from 1970-2023. The 2024 run estimates are shown by the coloured lines. Each line represents the proportion through based on a final season estimate shown in the legend. For example, the red line is based on a final TRTC of 1.5M, with the line showing the cumulative number of sockeye through on each date / 1.5M. Until we have the true final TRTC, we will not be able to calculate the true cumulative proportion. This is done to give an idea of run-timing and whether the pre-season forecast is likely to be close to the in-season estimate. The blue dashed line indicates the long-term average 50% through date."
#| fig-height: 5

dailycum<-read_excel("data/tyee_indices_56_2023_allspecies.xlsx", sheet = "Chum")

gg.daily.pcum<-dailycum%>%select("Date":"2023")%>%
  mutate_if(is.character, as.numeric) %>%
  pivot_longer("1970":"2023",names_to="Year",values_to="Chum")%>%
  mutate(Year=as.numeric(Year))
...
ggplot(gg.daily.pcum,aes(x=Date,y=Chum,group=Date))+
  geom_boxplot(fill="white",color="grey70",alpha=.5)+
    geom_line(data=data2,aes(x=as.POSIXct(date),y=p,color=estimate,group=estimate),linewidth=1,linewidth=1)+
    geom_segment(aes(x=as.POSIXct("2024-06-01"),y=.5,xend=as.POSIXct("2024-07-24"),yend=.5),color="blue",linewidth=1,linetype="dashed")+
  geom_segment(aes(x=as.POSIXct("2024-07-24"),y=.5,xend=as.POSIXct("2024-07-24"),yend=0),color="blue",arrow = arrow(length=unit(.25, 'cm')),linewidth=1,linetype="dashed")+
...
#| message: false
#| warning: false
#| fig-cap: "Daily run for Pink salmon on even-numbered years through Tyee (uncorrected) from 1956-2022. 2022 run is shown in purple."
#| fig-height: 5

daily<-read_excel("data/tyee_indices_56_2023_allspecies.xlsx", sheet = "Pink.Even")
fence<-read_excel("data/tyee and catch and fence in-season updates 2024.xlsx",sheet="Babine")
fence$Date<-as.Date(fence$Date)

gg.daily<-daily%>%
  mutate_if(is.character, as.numeric) %>%
  pivot_longer("1956":"2022",names_to="Year",values_to="Pink salmon") %>%
  mutate(Year=as.numeric(Year),
         Date = as.Date(paste("2022", MONTH, DAY, sep="-"), "%Y-%m-%d"))

ggplot(gg.daily,aes(x=Date,y=`Pink salmon`,group=Date))+
  geom_boxplot(fill="white",alpha=.5)+
      geom_line(data=gg.daily%>%filter(Year==2022),aes(x=Date,y=`Pink salmon`,group=1),
                color="purple",linewidth=1)+
scale_x_date(date_labels="%b", date_breaks="1 month") +
  theme_bw()
```
...
#| message: false
#| warning: false
#| fig-cap: "Daily cumulative proportion of the Pink salmon run through Tyee (uncorrected) from 1970-2023. The 2024 run estimates are shown by the coloured lines. Each line represents the proportion through based on a final season estimate shown in the legend. For example, the red line is based on a final TRTC of 1.5M, with the line showing the cumulative number of sockeye through on each date / 1.5M. Until we have the true final TRTC, we will not be able to calculate the true cumulative proportion. This is done to give an idea of run-timing and whether the pre-season forecast is likely to be close to the in-season estimate. The blue dashed line indicates the long-term average 50% through date."
#| fig-height: 5

dailycum<-read_csv("data/tyee_indices_to2022_pink_even.csv")

gg.daily.pcum<-dailycum%>%select("Date":"2022")%>%
  mutate_if(is.character, as.numeric) %>%
  pivot_longer("1970":"2022",names_to="Year",values_to="Pink Salmon")%>%
  mutate(Year=as.numeric(Year))
...
ggplot(gg.daily.pcum,aes(x=Date,y=`Pink Salmon`,group=Date))+
  geom_boxplot(fill="white",color="grey70",alpha=.5)+
    geom_line(data=data2,aes(x=as.POSIXct(date),y=p,color=estimate,group=estimate),linewidth=1,linewidth=1)+
    geom_segment(aes(x=as.POSIXct("2024-06-01"),y=.5,xend=as.POSIXct("2024-07-24"),yend=.5),color="blue",linewidth=1,linetype="dashed")+
  geom_segment(aes(x=as.POSIXct("2024-07-24"),y=.5,xend=as.POSIXct("2024-07-24"),yend=0),color="blue",arrow = arrow(length=unit(.25, 'cm')),linewidth=1,linetype="dashed")+
...
#| message: false
#| warning: false
#| fig-cap: "Daily run for Pink salmon on odd-numbered years through Tyee (uncorrected) from 1957-2023. 2023 run is shown in purple."
#| fig-height: 5

daily<-read_excel("data/tyee_indices_56_2023_allspecies.xlsx", sheet = "Pink.Odd")
...
ggplot(gg.daily,aes(x=Date,y=`Pink salmon`,group=Date))+
  geom_boxplot(fill="white",alpha=.5)+
      geom_line(data=gg.daily%>%filter(Year==2023),aes(x=Date,y=`Pink salmon`,group=1),
                color="purple",linewidth=1)+
scale_x_date(date_labels="%b", date_breaks="1 month") +
  theme_bw()
```
...
dailycum<-read_csv("data/tyee_indices_to2023_pink_odd.csv")

gg.daily.pcum<-dailycum%>%select("Date":"2023")%>%
  mutate_if(is.character, as.numeric) %>%
  pivot_longer("1971":"2023",names_to="Year",values_to="Pink Salmon")%>%
  mutate(Year=as.numeric(Year))
...
#| message: false
#| warning: false
#| fig-cap: "Daily run for Coho salmon through Tyee (uncorrected) from 1956-2023. 2023 run is shown in purple."
#| fig-height: 5

daily<-read_excel("data/tyee_indices_56_2023_allspecies.xlsx", sheet = "Coho")

gg.daily<-daily%>%
  mutate_if(is.character, as.numeric) %>%
  pivot_longer("1956":"2023",names_to="Year",values_to="Coho") %>%
  mutate(Year=as.numeric(Year),
         Date = as.Date(paste("2023", MONTH, DAY, sep="-"), "%Y-%m-%d"))

ggplot(gg.daily,aes(x=Date,y=Coho,group=Date))+
  geom_boxplot(fill="white",alpha=.5)+
      geom_line(data=gg.daily%>%filter(Year==2023),aes(x=Date,y=Coho,group=1),
                color="purple",linewidth=1)+
scale_x_date(date_labels="%b", date_breaks="1 month") +
  theme_bw()
```
...
#| message: false
#| warning: false
#| fig-cap: "Daily cumulative proportion of the Coho run through Tyee (uncorrected) from 1970-2023. The 2024 run estimates are shown by the coloured lines. Each line represents the proportion through based on a final season estimate shown in the legend. For example, the red line is based on a final TRTC of 1.5M, with the line showing the cumulative number of sockeye through on each date / 1.5M. Until we have the true final TRTC, we will not be able to calculate the true cumulative proportion. This is done to give an idea of run-timing and whether the pre-season forecast is likely to be close to the in-season estimate. The blue dashed line indicates the long-term average 50% through date."
#| fig-height: 5

dailycum<-read_csv("data/tyee_indices_to2023_coho.csv")

gg.daily.pcum<-dailycum%>%select("Date":"2023")%>%
  mutate_if(is.character, as.numeric) %>%
  pivot_longer("1970":"2023",names_to="Year",values_to="Coho")%>%
  mutate(Year=as.numeric(Year))
...
ggplot(gg.daily.pcum,aes(x=Date,y=`Coho`,group=Date))+
  geom_boxplot(fill="white",color="grey70",alpha=.5)+
    geom_line(data=data2,aes(x=as.POSIXct(date),y=p,color=estimate,group=estimate),linewidth=1,linewidth=1)+
    geom_segment(aes(x=as.POSIXct("2024-06-01"),y=.5,xend=as.POSIXct("2024-07-24"),yend=.5),color="blue",linewidth=1,linetype="dashed")+
  geom_segment(aes(x=as.POSIXct("2024-07-24"),y=.5,xend=as.POSIXct("2024-07-24"),yend=0),color="blue",arrow = arrow(length=unit(.25, 'cm')),linewidth=1,linetype="dashed")+
