...
tyee.day<-as.Date("2025-08-26")
tyee.date<-"August 26"
figures.x.date<-as.Date("2025-09-15")

fence.day<-as.Date("2025-08-26")
fence.date<-"August 26"

today<-as.Date("2025-08-27")
```
...
-   Tyee Test Fishery
    ...
    -   Sockeye escapement estimates are tracking just below the median of returns from 1970-2024, with the TRTC model estimate stabilising over the last 2 weeks while decreasing slightly to around 1.66M on average timing (1.72M if one week late, and 1.63M if one week early), but still well below the pre-season forecast for early and average run-timing projections.
    -   If average run-timing, \~ 97.5% of the run has passed Tyee to date.
