...
tyee.day<-as.Date("2025-08-18")
tyee.date<-"August 18"
figures.x.date<-as.Date("2025-09-01")

fence.day<-as.Date("2025-08-18")
fence.date<-"August 18"

today<-as.Date("2025-08-19")
```
...
-   Tyee Test Fishery
    ...
    -   Sockeye escapement estimates are tracking just below the median of returns from 1970-2024, with the TRTC model estimate stabilising over the last 2 weeks while decreasing slightly to around 1.69M on average timing (1.81M if one week late, and 1.63M if one week early), but still well below the pre-season forecast for early and average run-timing projections.
    -   If average run-timing, \~ 94% of the run has passed Tyee to date.
...
babine.figures.x.high<-as.Date("2025-09-01")
```
