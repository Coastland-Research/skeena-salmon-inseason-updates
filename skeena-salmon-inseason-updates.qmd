...
tyee.day<-as.Date("2024-07-09")
tyee.date<-"July 9"
fence.day<-as.Date("2024-07-09")
fence.date<-"July 9"
```
...
-   To date 284.494 sockeye are estimated to have passed by Tyee, with about 12.7% of the run through to date on average (5.2% if 1-week late, 26.5% if one week early). Sockeye escapement has picked up over the last week and is now tracking well above the median daily run, with a large escapement yesterday (Figures 1-5).

-   We are getting closer to the mid-point of the run, but at just over 10% on average timing, it is still very early to make predictions on the final TRTC, however, the excel model is increasing for all 3 run-timings, suggesting that the run may be coming in later than average. The current run size prediction is 2.2M on average run-timing (4.8M if one week late and 1.2M if 1 week early) (Figure 6).
...
-   The Kitwanga fence started operations last week, with counts starting.
