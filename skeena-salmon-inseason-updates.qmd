...
tyee.day<-as.Date("2024-07-23")
tyee.date<-"July 23"
fence.day<-as.Date("2024-07-23")
fence.date<-"July 23"
today<-as.Date("2024-07-23")
```
...
-   Tyee Test Fishery
    ...
    -   To date 906,669 sockeye are estimated to have passed by Tyee, with about 46.2% of the run through to date on average (26% if 1-week late, 67% if one week early). Sockeye escapement is not picking up for a large run and is continuing to be variable (Figures 1-5). Including commercial catch and First Nations marine catches, the TRTC estimate around 956,000 (not including marine food fish and catch from the last few days).

    -   Nearing the midpoint of the run on average timing, we are gaining more confidence in predicting the final TRTC. The excel model increased in early July stabilized briefly, decreased significantly, and stabilized again, and is now slightly increasing with the last couple days of large runs. We are now tracking right around the 2023 run.

    -   The current run size prediction is 2.0M on average run-timing (3.2M if one week late and 1.5M if 1 week early) (Figure 6). The next week will be important towards the peak of the run on avearge run timing, and will tell if we are tracking a 2022, 2023 or a something in between run. We should be seeing daily runs of aroudn 50-100k to keep pace with the 2023 return.

-   Babine Sockeye

    -   The Babine River fence started operations on July 9th. To July 22 the total run is 21,800 large sockeye, with 366 jack sockeye to date. More details will be added here shortly along with 2 forecast methods.
...
-   Commercial Fisheries
    ...
    -   Commercial fishing has started, with an Area C gillnet openings on July 13, 20 and 21, and First Nations demonstration fisheries on July 16. Preliminary sockeye catch in the Area C gillnet openings is ~ 37,000. Preliminary demonstration fishery catches are ~ 2,000 sockeye.
...
#| warning: false
#| message: false
#| fig-cap: "Daily run of sockeye to the Babine fence since 2020. 2024 run is shown in purple."
...
ggplot(babine.sx.2,aes(x=Date,y=sockeye,color=factor(year),linewidth=period,alpha=period))+
  geom_line()+
  scale_linewidth_manual(values=c(2,1))+
  scale_alpha_manual(values=c(1,.5))+
  theme_bw()+
...
ggplot(babine.sx.2,aes(x=Date,y=cum_sum,color=factor(year),linewidth=period,alpha=period))+
  geom_line()+
  scale_linewidth_manual(values=c(2,1))+
  scale_alpha_manual(values=c(1,.5))+
  theme_bw()+
