...
tyee.day<-as.Date("2024-08-29")
tyee.date<-"August 29"
fence.day<-as.Date("2024-08-29")
fence.date<-"August 29"
today<-as.Date("2024-08-29")
```
...
-   Babine Fence
    ...
    -   The Babine River fence started operations on July 9th. The total large sockeye run to date is \~ 1.8M with recent large return days bumping the run over the recent and historical averages, and exceeding 2023, with relatively strong days continuing (Figures 12-14).
...
demo_marine <- fn_catch %>%
  filter(nation %in% c("Lax Kw'alaams", "NCS"))
...
t<-data$cumtyee[i] + s + gn + dm + 30000

stat3<-data.frame(`Cumulative to Date`=c("Escapement Past Tyee", "Area A Seine Catch", "Area C Gillnet Catch", "First Nations Demo Catch (Marine)", "First Nations Demo Catch (In-River)", "Total First Nations FSC Catch", "TRTC (Escapement+Catch)"),`Number of Sockeye`=c(e, s, gn, dm, di, fsc, t))
names(stat3) <- gsub('[.]', ' ', names(stat3))

kable(stat3,caption=paste0("Cumulative escapement, catch, and TRTC to ",tyee.date,"."))
```
...
stat4 <- data.frame(`Cumulative to Date`=c("Escapement past Tyee", "TRTC (Escapement + Catch)", "Catch (preliminary)"), `Run Timing`=c("Average","One week early","One week late"),`Number of Sockeye`=c(e, t, c), `Percent Through`=c(avgper,earlyper,lateper))
names(stat4) <- gsub('[.]', ' ', names(stat4))

kable(stat4, caption=paste0("Run timing to ",tyee.date,"."))
```
