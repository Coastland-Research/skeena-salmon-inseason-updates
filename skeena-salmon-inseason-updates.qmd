...
tyee.day<-as.Date("2025-07-22")
tyee.date<-"July 22"
figures.x.date<-as.Date("2025-08-01")

fence.day<-as.Date("2025-07-22")
fence.date<-"July 22"

today<-as.Date("2025-07-23")
```
...
-   Tyee Test Fishery
    ...
    -   Sockeye escapement estimates are tracking right around the median of returns from 1970-2024, with the TRTC model estimate stabilising over the last week to around 1.7M on average timing (3.3M if one week late, and 1.1M if one week early), but still well below the pre-season forecast for early and average run-timing projections.
    ...
    -   Pink salmon have started entering the Skeena, and are tracking off the charts well above the highest run index ever.
...
make.index.figures.pink(daily,current,150,1000,figures.x.date)
```
...
babine.daily<-make.babine.daily.plot(gg.daily,gg.daily.quants,as.Date("2025-08-01"),100)
babine.cum<-make.babine.cum.plot(as.Date("2025-08-01"),300)
...
babine.daily<-make.babine.daily.plot(gg.daily,gg.daily.quants,as.Date("2025-08-01"),10000)
babine.cum<-make.babine.cum.plot(as.Date("2025-08-01"),100000)
