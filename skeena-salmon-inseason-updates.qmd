...
gg.daily.quants<-gg.daily%>%
  group_by(Date)%>%
  summarise(quantile=scales::percent(c(.1,.25,.75,.9)),per=quantile(Chinook,c(.1,.25,.75,.9),na.rm=TRUE))
...
ggplot(gg.daily,aes(x=Date,y=Chinook,group=Date))+
  geom_boxplot(color="grey80")+
    geom_line(data=gg.daily.quants,aes(x=Date,y=per,color=quantile,group=quantile)
                ,linewidth=1)+
    geom_line(data=gg.daily%>%filter(Year==2024),aes(x=Date,y=Chinook,group=1),
...
ggplot(gg_daily_cum,aes(x=Date,y=Chinook,group=Date))+
  geom_boxplot(fill="white",color="grey70",alpha=.5)+
    #geom_line(data=data2,aes(x=as.POSIXct(date),y=p,color=estimate,group=estimate),linewidth=1)+
    #geom_segment(aes(x=as.POSIXct("2023-06-01"),y=.5,xend=as.POSIXct("2023-07-24"),yend=.5),color="blue",linewidth=1,linetype="dashed")+
  #geom_segment(aes(x=as.POSIXct("2023-07-24"),y=.5,xend=as.POSIXct("2023-07-24"),yend=0),color="blue",arrow = arrow(length=unit(.25, 'cm')),linewidth=1,linetype="dashed")+
    theme_bw()+
