...
tyee.day<-as.Date("2024-07-08")
tyee.date<-"July 8"
fence.day<-as.Date("2024-07-08")
fence.date<-"July 8"
```
...
-   To date 217,964 sockeye are estimated to have passed by Tyee, with about 11.3% of the run through to date on average (4.6% if 1-week late, 24.2% if one week early). Sockeye escapement has picked up over the last week and is now tracking well above the median daily run (Figures 1-5).

-   We are getting closer to the mid-point of the run, but at just over 10% on average timing, it is still very early to make predictions on the final TRTC, however, the excel model is increasing for all 3 run-timings, suggesting that the run may be coming in later than average. The current run size prediction is 1.9M on average run-timing (Figure 6).
...
Currently, daily run estimates are tracking close to the last 2 years, and higher than 2020 and 2021. See the next figure that shows the cumulative run through and the full season totals (unadjusted).
...
Currently, the 2024 run is tracking close to the last 2 years, and higher than 2020 and 2021.
