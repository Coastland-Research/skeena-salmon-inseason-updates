...
-   Tyee Test Fishery
    ...
    -   To date 1,106,184 sockeye are estimated to have passed by Tyee, with about 52% of the run through to date on average (32% if 1-week late, 72% if one week early). Sockeye escapement is not picking up for a large run and is continuing to be variable (Figures 1-5). Including commercial catch and First Nations marine catches, the TRTC estimate around 1,144,000 (not including marine food fish and catch from the last few days).
    ...
    -   The current run size prediction is 2.2M on average run-timing (3.3M if one week late and 1.7M if 1 week early) (Figure 6). The next week will be important towards the peak of the run on avearge run timing, and will tell if we are tracking a 2022, 2023 or a something in between run. We should be seeing daily runs of aroudn 50-100k to keep pace with the 2023 return.
