...
tyee.day<-as.Date("2024-07-21")
tyee.date<-"July 21"
fence.day<-as.Date("2024-07-21")
fence.date<-"July 21"
today<-as.Date("2024-07-21")
```
...
-   Tyee Test Fishery
    ...
    -   Tides influence the migration of salmon into the Skeena River. We are currently at large tides for the next few days and hopefully will see that drive large numbers of fish in. [(Tides section)](#tide-information-at-port-edward)

    -   To date 741,421 sockeye are estimated to have passed by Tyee, with about 40.2% of the run through to date on average (22% if 1-week late, 61% if one week early). Sockeye escapement is not picking up for a large run and is continuing to be variable (Figures 1-5). Including commercial catch and First Nations marine catches, the TRTC estimate around 770,000 (not including marine food fish and catch from the last few days).

    -   Nearing the midpoint of the run on average timing, we are gaining more confidence in predicting the final TRTC. The excel model increased in early July stabilized briefly, decreased significantly, and has stabilized again. We are still tracking just below the 2023 run.

    -   The current run size prediction is 1.9M on average run-timing (3.1M if one week late and 1.3M if 1 week early) (Figure 6). The next week will be important towards the peak of the run on avearge run timing, and will tell if we are tracking a 2022, 2023 or a something in between run. We should be seeing daily runs of between 30,000 and 60,000 to keep pace with the 2023 return.

-   Babine Sockeye

    -   The Babine River fence started operations on July 9th. To date the total run is 10,351 large sockeye, with 166 jack sockeye to date. More details will be added here shortly along with 2 forecast methods.
...
-   Tyee Other Species
    ...
    -   Steelhead are coming through Tyee and the index suggests a strong run to start, around the 90th percentile.
...
-   Commercial Fisheries
    ...
    -   Commercial fishing has started, with an Area C gillnet opening on July 13 - catch numbers will be updated shortly but preliminary sockeye catch is 7,173 pieces. There were also openings over the weekend on July 20 and 21.

-   Appendix A will include information on the other species caught in the Area 4 commercial fishery and CPUEs and will be updated shortly.
...
fencedaily<-read_excel("data/babine fence counts 1946-2024 compiled 20240717.xlsx",sheet='Large Sockeye')

fence2024<-read_excel("data/tyee and catch and fence in-season updates 2024.xlsx",sheet="Babine")%>%
  select(Date,`2024`=totalrun)%>%mutate(Date = as.Date(Date))

daily.babine<-left_join(fencedaily,fence2024,by="Date")%>%
...
b1<-ggplot(daily.babine%>%filter(period!="2024"),aes(x=Date,y=sockeye,color=period))+
  geom_line(linetype="longdash",alpha=.5)+
  geom_line(data=daily.babine%>%filter(period=="2024"),aes(x=Date,color="2024"),alpha=.75)+
  geom_point(data=daily.babine%>%filter(period=="2024"),aes(x=Date,color="2024"),
             size=1,pch=21,fill="purple",alpha=.75)+
  scale_color_manual(values=c("darkgreen","blue","purple","firebrick"))+
    theme_bw()+
...
#| echo: false
#| warning: false
#| fig-cap: "Top panel: Total run of large sockeye (including harvest at the fence) to the Babine fence for 2024 (purple line) compared to historical and recent averages." 
#| include: true
#| fig-height: 5

#Bottom panel: Cumulative estimated run to the Babine fence for 2024 and the average from 2011-2021."
b1
...
#  summarise(quantile10 = scales::percent(.1), 
 ...
 #           per10 = quantile(Coho,.1, na.rm = TRUE), 
...
#  geom_line(data = gg.daily %>% filter(Year == 2024), aes(x = Date, y = Coho, #group = 1, color = "2024 Data"), linewidth = 1) +
...
#| message: false
#| warning: false
#| fig-cap: "Daily run for steelhead through Tyee (uncorrected) from 1956-2024. 2024 run is shown in purple."
#| fig-height: 5
...
ggplot(gg.daily,aes(x=Date, y=Steelhead,group=Date))+
  geom_line(data=gg.daily.quants,aes(colour=qgroup,group=Q),linetype="longdash",linewidth=1)+
...
#| message: false
#| warning: false
#| fig-cap: "Daily cumulative proportion of the steelhead run through Tyee (uncorrected) from 1970-2023. The 2024 run estimates are shown by the coloured lines. Each line represents the proportion through based on a final season estimate shown in the legend. For example, the red line is based on a final TRTC of 1.5M, with the line showing the cumulative number of sockeye through on each date / 1.5M. Until we have the true final TRTC, we will not be able to calculate the true cumulative proportion. This is done to give an idea of run-timing and whether the pre-season forecast is likely to be close to the in-season estimate. The blue dashed line indicates the long-term average 50% through date."
#| fig-height: 5
...
The following section presents available information on catches in the Area 4 commercial marine gillnet and seine fisheries targeting sockeye at the mouth of the Skeena River (generally sub-areas 4-12 and 4-15). Information will be preliminary until catch is reviewed by DFO staff. First Nations demonstration fisheries catch is shown in the next section.
