---
title: "Skeena Salmon In-Season Updates 2024"
subtitle: "October 7, 2024"
format: html
editor: visual
editor_options: 
...
com_catch <- read_excel("data/2024_archive/commercial catch 2024.xlsx", sheet = "catch") %>%
...
fn_catch <- read_excel("data/2024_archive/FNs catch 2024.xlsx", sheet = "demo catches")
```
...
-   Tyee Test Fishery
    ...
    -   Tides can influence the migration of salmon into the Skeena River.

    -   To date \> 1.7M sockeye are estimated to have passed by Tyee, with very few additional sockeye expected. Daily sockeye counts have dropped and are now very low (Figures 1-5). Including commercial marine catch and First Nations marine FSC catch estimates, the TRTC estimate is around 1.97M with just under 200,000 marine commercial catch between Area C, Area A and First Nations Demo/EO fisheries.
    ...
    -   The current run size prediction is about 1.9M for all three run-timings (Figure 6).
    ...
    -   The Steelhead index suggested a very strong run at the beginning of the season, however daily counts fell off slightly and the run is currently tracking near the 75th percentile. The latest Province of BC update estimates around 34,976 Steelhead through Tyee to date (as of September 10) (Figure 11).

-   Babine Fence and large sockeye

    -   The Babine River fence started operations on July 9th. The total large sockeye run to date is nearing 2 million, well over the recent and historical averages, and exceeding 2023, with relatively strong days continuing (Figures 12-14).
...
-   Babine Jack sockeye
    ...
    -   The total run of jack sockeye to date is now over 125k, well above the longer term and recent averages (Figures 16-17).
...
-   Kitwanga Fence
    ...
    -   Details on the Kitwanga fence counts can be found at the [Gitanyow Fisheries Authority website](http://www.gitanyowfisheries.com/).

-   Commercial Fisheries

    -   Commercial fishing has started, with Area C gillnet openings on July 13, 20-21, 25-26, 29-30, August 2-3 and 8-9. The Area A seine fishery was open July 25-28 and August 2-5. Coastal Demonstration Fisheries opened on July 16, 21, 24, 27-28, July 31-August 2, August 4-7 and 11. In-river demonstration fisheries started July 30 for the Gitksan and August 15 for LBN. An ESSR fishery at Fulton River targeting sockeye opened on August 23 and August 30 for the Lake Babine Nation. There was also an ESSR fishery targeting pink salmon at Witset.
...
-   Recreational Fisheries
    ...
    -   Current regulations for Region 6 (Skeena) can be found on [DFO's website here](https://www.pac.dfo-mpo.gc.ca/fm-gp/rec/fresh-douce/region6-eng.html).
...
seine <- com_catch %>%
  filter(gear == "seine" & sp == "Sockeye")
s <- sum(seine$Catch, na.rm = TRUE)

gnet <- com_catch %>%
  filter(gear == "gillnet" & sp == "Sockeye")
gn <- sum(gnet$Catch, na.rm=TRUE)

demo_marine <- fn_catch %>%
  filter(nation %in% c("Lax Kw'alaams", "NCS", "Metlakatla"))
dm <- sum(demo_marine$pieces, na.rm=TRUE)

demo_inriver <- fn_catch %>%
  filter(nation %in% c("Gitksan", "Lake Babine Nation"))
di <- sum(demo_inriver$pieces, na.rm=TRUE)
...
```{r load babine data, include=FALSE}
babine_fence <- read_excel("data/babine data 2024.xlsx", sheet = "Babine") %>% mutate(Date=as.Date(Date))
```
...
i<-match(fence.day,babine_fence$Date)
bab_sockeye <- babine_fence$cumesc[i]

# 3. Total catch below Babine
tc_below <- s + gn + di + dm + 75000
...
daily<-read_excel("data/Tyee data 2024.xlsx",sheet="tyee daily")
...
dailycum<-read_excel("data/Tyee data 2024.xlsx",sheet="tyee pcum")
...
daily.cum<-read_excel("data/Tyee data 2024.xlsx",sheet="tyee cum")
...
daily<-read_excel("data/2024_archive/tyee_indices_56_2023_allspecies.xlsx", sheet = "Large Chinook")%>%select(-`month-day`)
cn2024<-read_excel("data/2024_archive/Tyee data 2024.xlsx",sheet="index")%>%
...
daily<-read_excel("data/2024_archive/tyee_indices_56_2023_allspecies.xlsx", sheet = "Coho")%>%select(-MONTH,-DAY)
cn2024<-read_excel("data/2024_archive/Tyee data 2024.xlsx",sheet="index")%>%
...
dailyplot_co <- ggplot(gg.daily,aes(x=Date, y=Coho,group=Date))+
  geom_line(data=gg.daily.quants,aes(colour=qgroup,group=Q),linetype="longdash",linewidth=1)+
  geom_line(data = gg.daily %>% filter(Year == 2024&Date<="2024-09-01"), aes(x = Date, y = Coho, group = 1, color = "2024 Data"), linewidth = 1.5,alpha=.7)+
...
The following updates were provided by the Province of BC via email. Click on the link below to download a PDF of Tyee steelhead updates to Sept 10. For any questions please contact Kenji Miyazaki at BC.

![](steelhead_sept10_24.pdf){width="80%" height="500"}

Tyee steelhead updates can be downloaded here:

[Skeena Summer Steelhead Update to Sept 10](Skeena_Summer_Steelhead_Update_to_Sept_10_24.xlsx)

[Skeena Summer Steelhead Update to August 29](Skeena%20Summer%20Steelhead%20Update%20to%20Aug%20_29_24.xlsx)

[Skeena Summer Steelhead Update to August 22](Skeena%20Summer%20Steelhead%20Update%20to%20Aug%20_22_24.xlsx)

[Skeena Summer Steelhead Update to August 15](Skeena%20Summer%20Steelhead%20Update%20to%20Aug%20_15_24.xlsx)

[Skeena Summer Steelhead Update to August 8](Skeena_Summer_Steelhead_Update_to_Aug_8_24.xlsx)

[Skeena Summer Steelhead Update to August 1](Skeena%20Summer%20Steelhead%20Update%20to%20Aug%201_24.xlsx)

[Skeena Summer Steelhead Update to July 25](Skeena%20Summer%20Steelhead%20Update%20to%20July%2025_24%20(1).xlsx)

[Skeena Summer Steelhead Update to July 17](Skeena%20Summer%20Steelhead%20Update%20to%20July%2017_24.xlsx)

# Babine Fence

The Babine fence was installed and fish tight on July 9. Data from the Babine fence is provided nightly via email from the LBN Fisheries program.

Throughout 2024, there were concerns at the Babine River fence about high temperatures and low water levels holding up passage of sockeye and other salmon through the fence. In 2024, LBN implemented the Babine River Collaborative Management Plan and has choosen to voluntarily move their harvest from the Babine fence to Babine Lake. For more details please contact Cassie Siebert (LBN Fisheries Director).

### Babine Sockeye

This section shows information on the 2024 return of large and jack sockeye to the Babine fence.
...
The following figures show the daily and cumulative run of large sockeye estimated to the Babine fence for the last 5 years. This figure shows how the run in 2024 compares with recent years in terms of the Babine sockeye.
...
#| warning: false
#| message: false
#| fig-cap: "Daily and cumulative run through the Babine fence (uncorrected) since 2020. 2024 run is shown in purple."
#| fig-height: 7
...
gg.daily.cum <- daily.index %>%
  ...
  pivot_longer("1956":"2024",names_to="Year",values_to="Large Sockeye") %>%
  group_by(Year)%>%
  mutate(cum_sum=cumsum(replace_na(`Large Sockeye`,0)))
...
gg.daily.cum.quants<-gg.daily.cum %>%
  ...
  pivot_longer("per10":"per90",names_to="Q",values_to="Index")%>%
  group_by(Q)%>%
  mutate(cum_sum=cumsum(replace_na(Index,0)))%>%
  mutate(qgroup=case_when(Q=="per10"|Q=="per90"~"10/90th",
                          Q=="per25"|Q=="per75"~"25/75th",
                          Q=="per50"~"Median"))
...
ty<-read_excel("data/Tyee data 2024.xlsx",sheet="Tyee") %>%
  mutate(Date = as.Date(Date))%>%
  select(Date, Observed.Tyee=cumtyee)

fe<-read_excel("data/babine data 2024.xlsx",sheet="Babine")%>%
  mutate(Date = as.Date(Date))%>%
...
dailyplot_jack <- ggplot(gg.daily,aes(x=Date, y=`Jack Sockeye`,group=Date))+
  ...
  geom_line(data = gg.daily %>% filter(Year <2024), aes(x = Date, y = `Jack Sockeye`,group=Year),
            linewidth = .5,alpha=.1)+
  scale_color_manual(values=c("grey50","purple","grey75","black"))+
  labs(y="Daily Sockeye Jacks",color="")+
  theme_bw()+
  theme(legend.position = "bottom") +
  ylim(0,20000)
...
cumplot_jack <- ggplot(gg.daily.cum,aes(x=Date, y=cum_sum,group=Date))+
  ...
  geom_line(data = gg.daily.cum %>% filter(Year <2024), aes(x = Date, y = cum_sum,group=Year),
            linewidth = .5,alpha=.1)+
  scale_color_manual(values=c("grey50","purple","grey75","black"))+
  labs(y="Cumulative Sockeye Jacks",color="")+
  theme_bw()+
  theme(legend.position = "bottom")+
  ylim(0,400000)
...
```{r daily boxplot babine chinook}
#| label: fig-daily-boxplot-babcn
#| echo: false
#| include: true
#| message: false
#| warning: false
...
daily<-read_excel("data/babine fence counts 1946-2024 compiled 20240717.xlsx",sheet='Large Chinook') %>% select(-Month,-Day)

cn2024<-read_excel("data/babine data 2024.xlsx",sheet="Babine") %>%
  select(Date,"2024"=`large chinook`) %>% mutate(Date = as.Date(Date))
...
gg.daily<-daily.index%>%
  mutate_if(is.character, as.numeric) %>%
  pivot_longer("1946":"2024",names_to="Year",values_to="Large Chinook") %>%
  mutate(Year=as.numeric(Year)) %>%
  mutate(Index=replace_na(`Large Chinook`,0))

gg.daily.quants<-gg.daily %>%
  group_by(Date) %>%
  summarise(per10 = quantile(`Large Chinook`,.1, na.rm = TRUE), 
            per25 = quantile(`Large Chinook`,.25, na.rm = TRUE), 
            per50 = quantile(`Large Chinook`,.50, na.rm = TRUE),
            per75 = quantile(`Large Chinook`,.75, na.rm = TRUE),
            per90 = quantile(`Large Chinook`,.9, na.rm = TRUE))%>%
  select(Date,per10,per25,per50,per75,per90)%>%
  pivot_longer("per10":"per90",names_to="Q",values_to="Index")%>%
  group_by(Q)%>%
  mutate(`Large Chinook` = (Index)) %>%
  mutate(qgroup=case_when(Q=="per10"|Q=="per90"~"10/90th",
                          Q=="per25"|Q=="per75"~"25/75th",
                          Q=="per50"~"Median"))

  
dailyplot_bab <- ggplot(gg.daily,aes(x=Date, y=`Large Chinook`,group=Date))+
  geom_line(data=gg.daily.quants,aes(colour=qgroup,group=Q),linetype="longdash",linewidth=1)+
  geom_line(data = gg.daily %>% filter(Year == 2024&Date<=today), aes(x = Date, y = `Large Chinook`, group = 1, color = "2024 Data"), linewidth = 1.5,alpha=.7)+
  geom_line(data = gg.daily %>% filter(Year <2024), aes(x = Date, y = `Large Chinook`,group=Year),
            linewidth = .5,alpha=.1)+
  scale_color_manual(values=c("grey50","purple","grey75","black"))+
  labs(y="Daily Chinook",color="")+
  theme_bw()+
  theme(legend.position = "bottom") +
  ylim(0,350)
...
```{r daily cumulative boxplot babine chinook}
#| label: fig-daily-cumulative-boxplot-babcn
#| echo: false
#| include: true
#| message: false
#| warning: false
...
gg.daily.cum <- daily.index %>%
  ...
  pivot_longer("1946":"2024",names_to="Year",values_to="Large Chinook") %>%
  group_by(Year)%>%
  mutate(cum_sum=cumsum(replace_na(`Large Chinook`,0)))
...
cumplot_bab <- ggplot(gg.daily.cum,aes(x=Date, y=cum_sum,group=Date))+
  ...
  geom_line(data = gg.daily.cum %>% filter(Year <2024), aes(x = Date, y = cum_sum,group=Year),
            linewidth = .5,alpha=.1)+
  scale_color_manual(values=c("grey50","purple","grey75","black"))+
  labs(y="Cumulative Chinook",color="")+
  theme_bw()+
  theme(legend.position = "bottom")+
  ylim(0,10000)
...
```{r}
#| label: fig-plots-babcn
#| echo: false
#| results: asis
#| message: false
#| warning: false
#| include: true
#| fig-cap: "Top panel: Daily run for large Chinook through the Babine fence (uncorrected) from 1946-2024. 2024 run is shown in purple. Bottom panel: Cumulative daily index of the Chinook salmon run through the Babine fence (uncorrected) from 1946-2024. The 2024 run estimate is shown by the purple line. Individual years are shown in light grey in the background. The p50 line is shown as a dark black dashed line, The p25 and p75 lines are lighter grey dashed lines. The p10 and p90 lines are shown in the light grey dashed lines. "
#| fig-height: 8
...
```{r daily boxplot babine jack chinook}
#| label: fig-daily-boxplot-babjc
#| echo: false
#| include: true
#| message: false
#| warning: false
...
daily<-read_excel("data/babine fence counts 1946-2024 compiled 20240717.xlsx",sheet='Jack Chinook') %>% select(-Month,-Day)

cn2024<-read_excel("data/babine data 2024.xlsx",sheet="Babine") %>%
  select(Date,"2024"=`jack chinook`) %>% mutate(Date = as.Date(Date))
...
gg.daily<-daily.index%>%
  mutate_if(is.character, as.numeric) %>%
  pivot_longer("1946":"2024",names_to="Year", values_to="Jack Chinook") %>%
  mutate(Year=as.numeric(Year)) %>%
  mutate(Index=replace_na(`Jack Chinook`,0))

gg.daily.quants<-gg.daily %>%
  group_by(Date) %>%
  summarise(per10 = quantile(`Jack Chinook`,.1, na.rm = TRUE), 
            per25 = quantile(`Jack Chinook`,.25, na.rm = TRUE), 
            per50 = quantile(`Jack Chinook`,.50, na.rm = TRUE),
            per75 = quantile(`Jack Chinook`,.75, na.rm = TRUE),
            per90 = quantile(`Jack Chinook`,.9, na.rm = TRUE))%>%
  select(Date,per10,per25,per50,per75,per90)%>%
  pivot_longer("per10":"per90",names_to="Q",values_to="Index")%>%
  group_by(Q)%>%
  mutate(`Jack Chinook` = (Index)) %>%
  mutate(qgroup=case_when(Q=="per10"|Q=="per90"~"10/90th",
                          Q=="per25"|Q=="per75"~"25/75th",
                          Q=="per50"~"Median"))

  
dailyplot_bab <- ggplot(gg.daily,aes(x=Date, y=`Jack Chinook`,group=Date))+
  geom_line(data=gg.daily.quants,aes(colour=qgroup,group=Q),linetype="longdash",linewidth=1)+
  geom_line(data = gg.daily %>% filter(Year == 2024&Date<=today), aes(x = Date, y = `Jack Chinook`, group = 1, color = "2024 Data"), linewidth = 1.5,alpha=.7)+
  geom_line(data = gg.daily %>% filter(Year <2024), aes(x = Date, y = `Jack Chinook`,group=Year),
            linewidth = .5,alpha=.1)+
  scale_color_manual(values=c("grey50","purple","grey75","black"))+
  labs(y="Daily Jack Chinook",color="")+
  theme_bw()+
  theme(legend.position = "bottom") +
  ylim(0,100)
...
```{r daily cumulative boxplot babine jack chinook}
#| label: fig-daily-cumulative-boxplot-babjc
#| echo: false
#| include: true
#| message: false
#| warning: false
...
gg.daily.cum <- daily.index %>%
  ...
  pivot_longer("1946":"2024",names_to="Year",values_to="Jack Chinook") %>%
  group_by(Year)%>%
  mutate(cum_sum=cumsum(replace_na(`Jack Chinook`,0)))
...
cumplot_bab <- ggplot(gg.daily.cum,aes(x=Date, y=cum_sum,group=Date))+
  ...
  geom_line(data = gg.daily.cum %>% filter(Year <2024), aes(x = Date, y = cum_sum,group=Year),
            linewidth = .5,alpha=.1)+
  scale_color_manual(values=c("grey50","purple","grey75","black"))+
  labs(y="Cumulative Jack Chinook",color="")+
  theme_bw()+
  theme(legend.position = "bottom")+
  ylim(0,2500)
...
```{r}
#| label: fig-plots-babjc
#| echo: false
#| results: asis
#| message: false
#| warning: false
#| include: true
#| fig-cap: "Top panel: Daily run for jack Chinook through the Babine fence (uncorrected) from 1946-2024. 2024 run is shown in purple. Bottom panel: Cumulative daily index of the jack Chinook run through the Babine fence (uncorrected) from 1946-2024. The 2024 run estimate is shown by the purple line. Individual years are shown in light grey in the background. The p50 line is shown as a dark black dashed line, The p25 and p75 lines are lighter grey dashed lines. The p10 and p90 lines are shown in the light grey dashed lines. "
#| fig-height: 8
...
```{r daily boxplot babine pink}
#| label: fig-daily-boxplot-babpk
#| echo: false
#| include: true
#| message: false
#| warning: false
...
daily<-read_excel("data/babine fence counts 1946-2024 compiled 20240717.xlsx",sheet='Pink') %>% select(-Month,-Day)

cn2024<-read_excel("data/babine data 2024.xlsx",sheet="Babine") %>%
  select(Date,"2024"=`pink`) %>% mutate(Date = as.Date(Date))
...
gg.daily<-daily.index%>%
  mutate_if(is.character, as.numeric) %>%
  pivot_longer("1956":"2024",names_to="Year",values_to="Pink") %>%
  mutate(Year=as.numeric(Year)) %>%
  mutate(Index=replace_na(`Pink`,0))

gg.daily.quants<-gg.daily %>%
  group_by(Date) %>%
  summarise(per10 = quantile(`Pink`,.1, na.rm = TRUE), 
            per25 = quantile(`Pink`,.25, na.rm = TRUE), 
            per50 = quantile(`Pink`,.50, na.rm = TRUE),
            per75 = quantile(`Pink`,.75, na.rm = TRUE),
            per90 = quantile(`Pink`,.9, na.rm = TRUE))%>%
  select(Date,per10,per25,per50,per75,per90)%>%
  pivot_longer("per10":"per90",names_to="Q",values_to="Index")%>%
  group_by(Q)%>%
  mutate(`Pink` = (Index)) %>%
...
dailyplot_bab <- ggplot(gg.daily,aes(x=Date, y=`Pink`,group=Date))+
  geom_line(data=gg.daily.quants,aes(colour=qgroup,group=Q),linetype="longdash",linewidth=1)+
  geom_line(data = gg.daily %>% filter(Year == 2024&Date<=today), aes(x = Date, y = Pink, group = 1, color = "2024 Data"), linewidth = 1.5,alpha=.7)+
  geom_line(data = gg.daily %>% filter(Year <2024), aes(x = Date, y = Pink,group=Year),
            linewidth = .5,alpha=.1)+
  scale_color_manual(values=c("grey50","purple","grey75","black"))+
  labs(y="Daily Pink",color="")+
  theme_bw()+
  theme(legend.position = "bottom") +
  ylim(0,20000)
...
```{r daily cumulative boxplot babine pink}
#| label: fig-daily-cumulative-boxplot-babpk
#| echo: false
#| include: true
#| message: false
#| warning: false
...
gg.daily.cum <- daily.index %>%
  ...
  pivot_longer("1956":"2024",names_to="Year",values_to="Pink") %>%
  group_by(Year)%>%
  mutate(cum_sum=cumsum(replace_na(`Pink`,0)))
...
```{r}
#| label: fig-plots-babpk
#| echo: false
#| results: asis
#| message: false
#| warning: false
#| include: true
#| fig-cap: "Top panel: Daily run for pink salmon through the Babine fence (uncorrected) from 1946-2024. 2024 run is shown in purple. Bottom panel: Cumulative daily index of the pink salmon run through the Babine fence (uncorrected) from 1946-2024. The 2024 run estimate is shown by the purple line. Individual years are shown in light grey in the background. The p50 line is shown as a dark black dashed line, The p25 and p75 lines are lighter grey dashed lines. The p10 and p90 lines are shown in the light grey dashed lines. "
#| fig-height: 8
...
```{r daily boxplot babine coho}
#| label: fig-daily-boxplot-babco
#| echo: false
#| include: true
#| message: false
#| warning: false
...
daily<-read_excel("data/babine fence counts 1946-2024 compiled 20240717.xlsx",sheet='Coho') %>% select(-Month,-Day)

cn2024<-read_excel("data/babine data 2024.xlsx",sheet="Babine") %>%
  select(Date,"2024"=`coho`) %>% mutate(Date = as.Date(Date))
...
```{r daily cumulative boxplot babine coho}
#| label: fig-daily-cumulative-boxplot-babco
#| echo: false
#| include: true
#| message: false
#| warning: false
...
```{r}
#| label: fig-plots-babco
#| echo: false
#| results: asis
#| message: false
#| warning: false
#| include: true
#| fig-cap: "Top panel: Daily run for coho salmon through the Babine fence (uncorrected) from 1946-2024. 2024 run is shown in purple. Bottom panel: Cumulative daily index of the coho salmon run through the Babine fence (uncorrected) from 1946-2024. The 2024 run estimate is shown by the purple line. Individual years are shown in light grey in the background. The p50 line is shown as a dark black dashed line, The p25 and p75 lines are lighter grey dashed lines. The p10 and p90 lines are shown in the light grey dashed lines. "
#| fig-height: 8
...
```{r daily boxplot babine steelhead}
#| label: fig-daily-boxplot-babst
#| echo: false
#| include: true
#| message: false
#| warning: false
...
daily<-read_excel("data/babine fence counts 1946-2024 compiled 20240717.xlsx",sheet='Steelhead') %>% select(-Month,-Day)

cn2024<-read_excel("data/babine data 2024.xlsx",sheet="Babine") %>%
  select(Date,"2024"=`steelhead`) %>% mutate(Date = as.Date(Date))
...
dailyplot_bab <- ggplot(gg.daily,aes(x=Date, y=`Steelhead`,group=Date))+
  geom_line(data=gg.daily.quants,aes(colour=qgroup,group=Q),linetype="longdash",linewidth=1)+
  geom_line(data = gg.daily %>% filter(Year == 2024&Date<=today), aes(x = Date, y = Steelhead, group = 1, color = "2024 Data"), linewidth = 1.5,alpha=.7)+
  geom_line(data = gg.daily %>% filter(Year <2024), aes(x = Date, y = Steelhead,group=Year),
            linewidth = .5,alpha=.1)+
  scale_color_manual(values=c("grey50","purple","grey75","black"))+
  labs(y="Daily Steelhead",color="")+
  theme_bw()+
  theme(legend.position = "bottom") +
  ylim(0,40)
...
```{r daily cumulative boxplot babine steelhead}
#| label: fig-daily-cumulative-boxplot-babst
#| echo: false
#| include: true
#| message: false
#| warning: false
...
cumplot_bab <- ggplot(gg.daily.cum,aes(x=Date, y=cum_sum,group=Date))+
  ...
  geom_line(data = gg.daily.cum %>% filter(Year <2024), aes(x = Date, y = cum_sum,group=Year),
            linewidth = .5,alpha=.1)+
  scale_color_manual(values=c("grey50","purple","grey75","black"))+
  labs(y="Cumulative Steelhead",color="")+
  theme_bw()+
  theme(legend.position = "bottom")+
  ylim(0,500)
...
```{r}
#| label: fig-plots-babst
#| echo: false
#| results: asis
#| message: false
#| warning: false
#| include: true
#| fig-cap: "Top panel: Daily run for steelhead through the Babine fence (uncorrected) from 1946-2024. 2024 run is shown in purple. Bottom panel: Cumulative daily index of the steelhead run through the Babine fence (uncorrected) from 1946-2024. The 2024 run estimate is shown by the purple line. Individual years are shown in light grey in the background. The p50 line is shown as a dark black dashed line, The p25 and p75 lines are lighter grey dashed lines. The p10 and p90 lines are shown in the light grey dashed lines. "
#| fig-height: 8
...
-   The Kitwanga fence was operational and counting as of July 10.
...
demo.data<-read_excel("data/FNs catch 2024.xlsx",sheet="demo catches")
...
#| fig-height: 4
#| 
kable(demo.table,caption="Preliminary Area 4 First Nations demo catches of sockeye (not complete numbers).")%>%
...
Recreational fisheries notices for the Skeena are shown in the Fisheries Openings Table above. Current regulations for Region 6 (Skeena) can be found on [DFO's website here](https://www.pac.dfo-mpo.gc.ca/fm-gp/rec/fresh-douce/region6-eng.html).
...
```{r table of skeena sockeye rec fishery guidelines, echo=FALSE, warning=FALSE, include=TRUE}
options(knitr.kable.NA = '')
guidelines<-read_excel("data/guidelines_rec_sockeye.xlsx", sheet = "Sheet1")

kable(guidelines,booktabs=TRUE, caption="Guidelines for management actions for recreational sockeye fisheries in the Skeena Watershed")%>%
add_header_above(c("Estimated Abundance"=1, "Daily Limits"=3)) %>%
kable_styling(latex_options = "HOLD_position")
...
# 2024 tyee data
data<-read_excel("data/Tyee data 2024.xlsx",sheet="Tyee") %>%
  mutate(Date = as.Date(Date))
...
```{r daily escapement by year since 2000 vs 2024}
#| label: fig-tyee-esc-years
#| include: true
#| echo: false
#| warning: false
#| fig-cap: "Tyee 2024 daily sockeye escapement vs daily escapement each year between 2000-2023."
#| fig-height: 8
...
geom_line(data = data, aes(x = Date, y = esctyee, color = "2024 daily sockeye escapement")) + # line for 2024 data
...
```{r cumulative escapement by year vs 2024}
#| label: fig-tyee-cum-years
#| include: true
#| echo: false
#| warning: false
#| fig-cap: "Tyee 2024 cumulative sockeye escapement vs cumulative escapement each year between 2000-2023."
#| fig-height: 8
...
othercatch<-read_excel("data/commercial catch 2024.xlsx",sheet="catch")
...
oc2<-othercatch%>%pivot_longer(4:14,names_to="Species",values_to="Catch")%>%
  mutate(cpue=Catch/effort)%>%
  filter(Date>as.Date("2024-07-05")&Date<as.Date("2024-08-10"))%>%mutate(dis=ifelse(Species%in%c("Chinook (Released)","Chum (Released)","Steelhead (Released)","Coho (Released)","Sockeye (Released)","Pink (Released)"),"Released","Kept"))%>%
  mutate(sp=case_when(Species=="Chinook (Released)"~"Chinook",
