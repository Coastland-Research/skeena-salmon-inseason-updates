...
todate<-as.Date("2025-07-30")
todate.label<-"July 30"
...
ggplot(data,aes(x=Date,y=p,group=Date,fill=isittoday))+
  ...
  annotate(geom="text",label=paste0("Median = ",med.today),x=as.Date("2025-06-05"),y=.9,hjust=0)+
  annotate(geom="text",label=paste0("p25 = ",p25),x=as.Date("2025-06-05"),y=.86,hjust=0)+
  annotate(geom="text",label=paste0("p75 = ",p75),x=as.Date("2025-06-05"),y=.82,hjust=0)+
...
ggsave("steelhead run proportion plot.png")
...
ggsave("steelhead index lm to july 30.png")
...
ggsave("steelhead escapement lm to july 30.png")
